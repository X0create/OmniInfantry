...
static inline float fm_mod_2pi(float x) {
    /* ����ģ2������ */
    const float inv_2pi = 0.15915494309189535f;  /* 1/(2��) */
...
float fm_sin_fast(float x) {
    ...
    const float B = 1.27323954f;      /* 4/�� */
    const float C = -0.405284735f;    /* -4/(��2) */
...
void fm_sincos_fast(float x, float* sin_val, float* cos_val) {
    ...
    *cos_val = fm_sqrt_fast(1.0f - s * s);
    ...
    float xm = fm_mod_2pi(x);
    if (xm > FM_PI_2 && xm < 3.0f * FM_PI_2) {
        *cos_val = -(*cos_val);
    }
}
...
float fm_atan_fast(float x) {
    ...
    if (absx < 1.0f) {
        ...
        y = FM_PI_2 - absx / (absx * absx + 0.28f);
        if (x < 0.0f) y = -y;
    }
...
float fm_log2_fast(float x) {
    ...
    u.u = (u.u & 0x7FFFFF) | 0x3F800000;  /* ��ָ����Ϊ0��β����[1,2) */
    
    /* β����[1,2)��Χ�ڣ�ʹ�ö���ʽ����log2 */

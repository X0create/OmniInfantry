...
#include "usart.h"
 ...
 uint8_t Image_Trans_MultiRx_Buff[2][39];
...
void Image_Transmission_Message_Moniter(Image_Transmission_Info_TypeDef  *Image_Transmission_Info)
{
  ...
  if(Image_Transmission_Info->remote_control.online_cnt <= IMAGE_TRANS_LOST_THRESHOLD)
  {
    ...
    /* set the lost flag */
		Image_Transmission_Info->remote_control.lost = true;
		
  }
  else if(Image_Transmission_Info->remote_control.online_cnt > 0)
  {
    /* online count decrements which reseted in received interrupt  */
    Image_Transmission_Info->remote_control.online_cnt--;
  }
}
...
void VT13_Message_Moniter(VT13_Info_TypeDef  *VT13_Info)
{
  ...
  else if(VT13_Info->online_cnt > 0)
  {
    /* online count decrements which reseted in received interrupt  */
    VT13_Info->online_cnt--;
  }
}
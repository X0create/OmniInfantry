...
float fm_sqrt_fast(float x) {
    ...
    /* Ê¹ÓÃ¸Ä½øµÄQuake IIIËã·¨ */
    fm_float_t u;
    u.f = x;
    u.u = 0x5f375a86 - (u.u >> 1);  /* ±ÈÔ­Ê¼³£Á¿¸ü¾«È· */
    
    /* Ò»´ÎÅ£¶Ùµü´ú */
    float xhalf = 0.5f * x;
    u.f = u.f * (1.5f - xhalf * u.f * u.f);
    
    /* ¸ù¾İ¾«¶ÈĞèÇó¾ö¶¨ÊÇ·ñ½øĞĞµÚ¶ş´Îµü´ú */
#if FM_PRECISE_MODE
    u.f = u.f * (1.5f - xhalf * u.f * u.f);
#endif
...
static inline float fm_mod_2pi(float x) {
    /* ¿ìËÙÄ£2¦ĞÔËËã */
    const float inv_2pi = 0.15915494309189535f;  /* 1/(2¦Ğ) */
    x -= (float)((int)(x * inv_2pi)) * FM_2PI;
    return x;
}

float fm_sin_fast(float x) {
    /* Ê¹ÓÃ¶àÏîÊ½½üËÆ£¬ÔÚ[-¦Ğ, ¦Ğ]·¶Î§ÄÚÎó²îĞ¡ÓÚ0.001 */
    x = fm_mod_2pi(x);
    
    /* Ê¹ÓÃ¸ü¾«È·µÄ7½×¶àÏîÊ½½üËÆ */
    const float B = 1.27323954f;      /* 4/¦Ğ */
    const float C = -0.405284735f;    /* -4/(¦Ğ2) */
...
float fm_atan_fast(float x) {
    ...
    if (absx < 1.0f) {
        ...
        /* ´ó½Ç¶ÈÊ¹ÓÃºãµÈÊ½ atan(x) = ¦Ğ/2 - atan(1/x) */
...
float fm_log2_fast(float x) {
    ...
    u.u = (u.u & 0x7FFFFF) | 0x3F800000;  /* ½«Ö¸ÊıÉèÎª0£¬Î²ÊıÔÚ[1,2) */
    
    /* Î²ÊıÔÚ[1,2)·¶Î§ÄÚ£¬Ê¹ÓÃ¶àÏîÊ½½üËÆlog2 */
...
/* é¿å…ç›´æ¥ä½¿ç”¨ <math.h> çš„ NAN å®ï¼šæœ¬å·¥ç¨‹ä¸º fast æµ®ç‚¹æ¨¡å‹ï¼Œ
   armclang ä¼šå¯¹å…¶å‘Šè­¦ -Wnan-infinity-disabledã€‚
   æ­¤å¤„æŒ‰ IEEE-754 ä½å‹æ„é€ åŒæ ·çš„å•ç²¾åº¦ quiet NaNï¼Œ
   è¿”å›å€¼ä¸åŸå…ˆé€ä½ç›¸åŒã€‚ */
...
float fm_rng_float(fm_rng_t* rng) {
    /* Éú³É[0,1)·¶Î§ÄÚµÄ¸¡µãÊı */
    uint32_t u = fm_rng_u32(rng);
    fm_float_t f;
    f.u = (u >> 9) | 0x3F800000;  /* Éú³É[1,2)·¶Î§ÄÚµÄ¸¡µãÊı */
    return f.f - 1.0f;
}
...
uint64_t fm_factorial(int n) {
    ...
    if (n < 0 || n > 20) return 0;  /* 20!ÊÇ64Î»ÎŞ·ûºÅÕûÊıµÄÉÏÏŞ */
    return factorials[n];
}
